...
class TWDiamondDifference3D : public BrickCellEquation3D
{
public:
             ...
             CertesianFaceSide z_uw, CertesianFaceSide z_dw) const override final
  {
    ...
    if (x_uw_af > 1e-8)
    {
      a = cell._current_iteration_source * V * theta_s + (abs_eta * B * y_uw_af + abs_xi * C * z_uw_af) * theta + abs_mu * A * x_uw_af;
    ...
    if (y_uw_af > 1e-8)
    {
      b = cell._current_iteration_source * V * theta_s + (abs_mu * A * x_uw_af + abs_xi * C * z_uw_af) * theta + abs_eta * B * y_uw_af;
    ...
    if (z_uw_af > 1e-8)
    {
      c = cell._current_iteration_source * V * theta_s + (abs_mu * A * x_uw_af + abs_eta * B * y_uw_af) * theta + abs_xi * C * z_uw_af;
    ...
    double center_af = cell._current_iteration_source * V + (abs_mu * A / a * x_uw_af) + (abs_eta * B / b * y_uw_af) + (abs_xi * C / c * z_uw_af);
