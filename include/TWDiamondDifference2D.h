...
public:
  ...
  void solve(CartesianCell2D & cell, const double & angular_weight,
             ...
             CertesianFaceSide y_uw, CertesianFaceSide y_dw) const override final
  {
    ...
    if (x_uw_af > 1e-8)
    {
      ...
      a /= (cell._sigma_t * V + 2.0 * abs_eta * B) * x_uw_af;
      a = -1.0 * (a - 1.0);
    }
    ...
    if (y_uw_af > 1e-8)
    {
      ...
      b /= (cell._sigma_t * V + 2.0 * abs_mu * A) * y_uw_af;
      b = -1.0 * (b - 1.0);
    }
    ...
    center_af /= cell._sigma_t * V + (abs_mu * A / a) + (abs_eta * B / b);
