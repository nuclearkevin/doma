...
TransportSolver3D<T>::TransportSolver3D(BrickMesh3D & mesh, const InputParameters & params, bool verbose)
  : _num_groups(params._num_e_groups),
    _mode(params._mode),
    _mesh(mesh),
    _angular_quad(2u * params._num_azimuthal, 2u * params._num_polar, 3u),
    _eq_system(),
    _verbose(verbose),
...
template <typename T>
bool
TransportSolver3D<T>::solveTransient(const std::string & output_file_base)
{
...
template <typename T>
bool
TransportSolver3D<T>::initSteadyIC()
{
  initializeSolve();
...
template <typename T>
bool
TransportSolver3D<T>::solveFixedSource(const std::string & output_file_base, const double & t)
{
...
TransportSolver3D<T>::computeMGFluxNorm()
{
  ...
  for (auto & cell : _mesh._cells)
    for (unsigned int g = 0u; g < _num_groups; ++g)
      norm += std::pow(cell._total_scalar_flux[g], 2.0) * cell._volume;
...
TransportSolver3D<T>::updateMultigroupSource(unsigned int g, double t)
{
  for (auto & cell : _mesh._cells)
  {
    ...
    cell._current_iteration_source = p._g_src.size() != 0u ? 0.25 * p._g_src[g] / M_PI : 0.0;
    ...
    if (_mode == RunMode::Transient && cell.hasStepSource())
    {
      ...
      {
        case StepType::Both:
          cell._current_iteration_source += ss._insert_time <= t && t <= ss._remove_time ? 0.25 * ss._g_src[g] / M_PI : 0.0;
          break;
        case StepType::Insert:
          cell._current_iteration_source += ss._insert_time <= t ? 0.25 * ss._g_src[g] / M_PI : 0.0;
          break;
        case StepType::Remove:
          cell._current_iteration_source += t <= ss._remove_time ? 0.25 * ss._g_src[g] / M_PI : 0.0;
          break;
      }
    }
    ...
    for (unsigned int g_prime = 0u; g_prime < _num_groups; ++g_prime)
    {
      ...
      if (p._g_chi_p.size() > 0u && p._num_d_groups == 0u)
        cell._current_iteration_source += 0.25 * p._g_chi_p[g] * p._g_prod[g_prime] * cell._total_scalar_flux[g_prime] / M_PI;
      else if (p._g_chi_p.size() > 0u && p._num_d_groups > 0u)
      {
        ...
        cell._current_iteration_source += 0.25 * (1.0 - g_beta) * p._g_chi_p[g] * p._g_prod[g_prime] * cell._total_scalar_flux[g_prime] / M_PI;
      }

      // Accumulate the contribution from delayed neutrons.
      if (p._num_d_groups > 0u && _mode == RunMode::Transient)
        for (unsigned int d = 0u; d < p._num_d_groups; ++d)
          cell._current_iteration_source += 0.25 * p._n_g_chi_d[g * p._num_d_groups + d] * cell._current_t_dnps[d] * p._n_lambda[d] / M_PI;

      // Accumulate the transient source.
      if (_mode == RunMode::Transient)
        cell._current_iteration_source += 0.25 * cell._last_t_scalar_flux[g] * p._g_inv_v[g] / _dt / M_PI;
    }
    cell._total_scalar_flux[g] = 0.0;
  }
}
